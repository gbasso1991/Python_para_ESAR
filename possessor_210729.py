...
def fourier_señales(t,v,v_r,delta_t,polaridad,filtro,frec_limite,name):
    ...
    #frecuencias multiplo impar de la fundamental
    frec_multip = []
    for n in range(int(frec_limite//int(armonicos[0]))):
        frec_multip.append((2*n+1)*armonicos[0]/1000)
    
    #Reconstruyo señal manera directa
    reconstruida = np.zeros_like(t)
    for n in range(len(armonicos)):
        reconstruida+= amplitudes[n]*np.cos(2*np.pi*armonicos[n]*t+ fases[n])
    reconstruida = reconstruida/max(reconstruida)
    
    #Reconstruyo señal con ifft
    g_aux = np.resize(g_aux,len(f))
    rec2 = irfft(g_aux,n=len(t),norm='forward')
    
    
    print('''Espectro de la señal de referencia: 
Frecuencia (Hz) - Intensidad rel - Fase (º)''')
    for i in range(len(indices_r)):
        print('{:<10.2f}    {:>12.2f}    {:>12.2f}'.format(armonicos_r[i],amplitudes_r[i]/max(amplitudes_r),fases_r[i]*180/np.pi))
    
    print('''\nEspectro de la señal de muestra: 
Frecuencia (Hz) - Intensidad rel - Fase (º)''')
    for i in range(len(indices)):
        print('{:<10.2f}    {:>12.2f}    {:>12.2f}'.format(armonicos[i],amplitudes[i]/max(amplitudes),fases[i]*180/np.pi))
  
    fig = plt.figure(figsize=(10,12),constrained_layout=True)
    ax = fig.add_subplot(3,1,1)
    ax.plot(t,y/max(y),'.-',lw=0.9,label='Muestra')
    ax.plot(t,y_r/max(y_r),'.-',lw=0.9,label='Referencia')
    ax.set_xlabel('t (s)')
    ax.set_xlim(0,2/armonicos[0])
    ax.axvspan(0,1/armonicos[0],color='g',alpha=0.3)
    ax.set_title('Muestra y referencia - '+str(name), loc='left', fontsize=13)
    plt.legend(loc='best')  
    plt.grid(True)
         
    ax1 = fig.add_subplot(3,1,2)
    ax1.plot(f/1000,g,'.-',lw=0.9,label = 'muestra')
    ax1.scatter(armonicos/1000,amplitudes,c='r',label='armónicos')
    for indx, item in enumerate(frec_multip):
        ax1.axvline(item,0,1,color='r',alpha=0.4,lw=0.9)
        
    ax1.set_title('Espectro de frecuencias - umbral: {}% - frec max: {:.0f} kHz'.format(filtro*100,frec_limite/1e3), loc='left', fontsize=13)
    ax1.set_xlabel('Frecuencia (kHz)')
    ax1.set_ylabel('|F{$\epsilon$}|')   
    ax1.set_xlim(0,max(f)/1000)

    axin1 = ax1.inset_axes([0.4, 0.35, 0.57, 0.6])
    axin1.scatter(armonicos/1000,fases,label='fases')
    axin1.set_ylabel('Frecuencia (kHz)')
    axin1.set_xlabel('Frecuencia (kHz)')
    axin1.set_ylim(-np.pi-0.5,np.pi+0.5)
    axin1.set_yticks([-np.pi,-np.pi/2,0,np.pi/2,np.pi])
    axin1.set_yticklabels(['-$\pi$','$\pi/2$','0','$\pi/2$','$\pi$'])
    axin1.axhline(0,0,max(armonicos)/1000,c='k',lw=0.8)
    for item in frec_multip:
        axin1.axvline(item,.1,0.87,color='r',alpha=0.4,lw=0.9)    
    axin1.set_ylabel('Fase')
    #axin1.legend(loc='best')
    axin1.set_title(' Espectro de fases',loc='left', y=0.87, fontsize=10)
    axin1.vlines(armonicos/1000, ymin=0, ymax=fases)
    axin1.set_xlim(0,max(f)/1000)
    #plt.legend(loc='best')  

    ax2 = fig.add_subplot(3,1,3)
    ax2.plot(t,y,'-',lw=0.9,label='Original')
    ax2.plot(t,reconstruida*max(rec2),'r-',lw=0.9,label='Reconstruida ({} armónicos)'.format(len(armonicos)))
    ax2.plot(t,rec2,'-',lw=0.9,label='Filtrada ({:.0f} kHz)'.format(frec_limite/1e3))
    ax2.set_xlabel('t (s)')
    ax2.set_xlim(0,2/armonicos[0])
    ax2.axvspan(0,1/armonicos[0],color='g',alpha=0.3)
    ax2.set_title('Señal de muestra - '+str(name), loc='left', fontsize=13)
    plt.legend(loc='best')  
    plt.grid(True)
    ...
    return armonicos, armonicos_r, amplitudes, amplitudes_r, fases , fases_r , fig
...
Analisis_de_Fourier = 0 # sobre las señales, imprime espectro de señal muestra
...
#%%
    ...
    if Analisis_de_Fourier == 1:
        #aplico la funcion
        armonicos_m,_,amplitudes_m,_,fases_m,_,fig_fourier = fourier_señales(t_m_3,Resta_m_3,v_r_m_3,delta_t[k],polaridad,filtro=0.03,frec_limite=4e6,name=fnames_m[k])

        fig_fourier.savefig('Analisis_Fourier_señales_{}_'.format(k)+str(fecha_nombre)+'.png',dpi=300)
